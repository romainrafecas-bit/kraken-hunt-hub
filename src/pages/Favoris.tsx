...
const Favoris = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data: favs } = await supabase
        .from("favorites")
        .select("product_url, collection, note, created_at")
        .eq("user_id", user.id);
      ...
      const prodMap = new Map((prods || []).map((p: any, i: number) => [p.url, mapToProduct(p, i)]));

      setFavorites(favs.map(f => ({
        product_url: f.product_url,
        collection: f.collection || "À sourcer",
        note: f.note || "",
        created_at: f.created_at,
        product: prodMap.get(f.product_url) || { id: 0, name: f.product_url, brand: "", category: "", price: 0, originalPrice: 0, recurrences: 0, lastSeen: "", rating: 0, score: 0, sellers: 0, image: "" },
      })));
      setLoading(false);
    };
    load();
  ...
  const removeFav = async (url: string) => {
    setFavorites(prev => prev.filter(f => f.product_url !== url));
    if (userId) await supabase.from("favorites").delete().eq("user_id", userId).eq("product_url", url);
  };

  const updateNote = async (url: string, note: string) => {
    setFavorites(prev => prev.map(f => f.product_url === url ? { ...f, note } : f));
    if (userId) await supabase.from("favorites").update({ note }).eq("user_id", userId).eq("product_url", url);
  };

  const moveToCollection = async (url: string, col: Collection) => {
    setFavorites(prev => prev.map(f => f.product_url === url ? { ...f, collection: col } : f));
    setMoveDropdown(null);
    if (userId) await supabase.from("favorites").update({ collection: col }).eq("user_id", userId).eq("product_url", url);
  };
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
            <AnimatePresence mode="popLayout">
              {filtered.map((fav, i) => {
                ...
                return (
                  <motion.div key={fav.product_url} layout initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9 }}
                    ...
                    <div className="absolute top-3 right-3 z-10 flex items-center gap-1.5">
                      {discount > 0 && <span className="bio-badge bio-teal text-[10px] py-0.5 px-2">-{discount}%</span>}
                    </div>
                    <div className="p-4">
                      ...
                      <div className="flex items-center gap-1.5 mt-3 pt-3 border-t border-border/50">
                        ...
                        <button onClick={() => setEditingNote(editingNote === fav.product_url ? null : fav.product_url)}
                        ...
                        <div className="relative">
                          <button onClick={() => setMoveDropdown(moveDropdown === fav.product_url ? null : fav.product_url)}
                            className="w-7 h-7 rounded-lg flex items-center justify-center bg-secondary/60 text-muted-foreground hover:text-foreground hover:bg-secondary transition-colors">
                            <ChevronDown className="w-3.5 h-3.5" />
                          </button>
                          {moveDropdown === fav.product_url && (
                            <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }}
                              ...
                              {collections.filter(c => c !== fav.collection).map(col => (
                                <button key={col} onClick={() => moveToCollection(fav.product_url, col)}
                        ...
                        <button onClick={() => removeFav(fav.product_url)}
                      ...
                      <AnimatePresence>
                        {editingNote === fav.product_url && (
                          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                            <textarea value={fav.note} onChange={e => updateNote(fav.product_url, e.target.value)} placeholder="Ajouter une note…"
                      ...
                      <div className="mt-2.5 space-y-1">
                        <p className="text-[10px] text-muted-foreground/60">Ajouté le {new Date(fav.created_at).toLocaleDateString("fr-FR", { day: "numeric", month: "long", year: "numeric" })}</p>
                        {fav.note && editingNote !== fav.product_url && (
